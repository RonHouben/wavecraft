...
use wavecraft_protocol::{
    DEV_PROCESSOR_VTABLE_VERSION, DEV_SIGNAL_CHAIN_SLOTS_SYMBOL, DevProcessorVTable, ParameterInfo,
    ProcessorInfo, SignalChainSlot,
};
...
type GetSignalChainSlotsJsonFn = unsafe extern "C" fn() -> *mut c_char;
...
pub struct PluginParamLoader {
    ...
    signal_chain_slots: Vec<SignalChainSlot>,
...
impl PluginParamLoader {
    ...
    pub fn load<P: AsRef<Path>>(dylib_path: P) -> Result<Self, PluginLoaderError> {
        ...
        // SAFETY: Symbol is generated by wavecraft_plugin! and uses GetSignalChainSlotsJsonFn ABI.
        let get_signal_chain_slots_json: Symbol<GetSignalChainSlotsJsonFn> = unsafe {
            library.get(DEV_SIGNAL_CHAIN_SLOTS_SYMBOL).map_err(|e| {
                PluginLoaderError::SymbolNotFound(format!(
                    "wavecraft_get_signal_chain_slots_json: {}",
                    e
                ))
            })?
        };

        let signal_chain_slots = load_json_via_ffi::<Vec<SignalChainSlot>>(
            *get_signal_chain_slots_json,
            *free_string,
            "wavecraft_get_signal_chain_slots_json",
        )?;
    ...
    /// Get the declared/default signal-chain slot order loaded from the plugin.
    pub fn signal_chain_slots(&self) -> &[SignalChainSlot] {
        &self.signal_chain_slots
    }
