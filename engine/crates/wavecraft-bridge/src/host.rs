...
pub trait ParameterHost: Send + Sync {
    ...
    /// Get the current processor order as slot indices (as strings).
    ///
    /// Returns an empty vec for hosts that don't support processor ordering.
    fn get_processor_order(&self) -> Vec<String> {
        vec![]
    }

    /// Set the desired processor order.
    ///
    /// Returns `Internal` error by default for hosts that don't support
    /// processor ordering. The real implementation is generated by the proc-macro
    /// on `__WavecraftParams` and forwarded through `PluginEditorBridge`.
    fn set_processor_order(&self, _order: &[String]) -> Result<(), BridgeError> {
        Err(BridgeError::Internal(
            "processor order not supported by this host".to_string(),
        ))
    }
}
...
impl<T: ParameterHost> ParameterHost for std::sync::Arc<T> {
    ...
    fn get_processor_order(&self) -> Vec<String> {
        forward_host(self).get_processor_order()
    }

    fn set_processor_order(&self, order: &[String]) -> Result<(), BridgeError> {
        forward_host(self).set_processor_order(order)
    }
}
...
/// Trait for accessing the runtime processor order from a parameter struct.
///
/// Implemented by `__WavecraftParams` (generated by the proc-macro) and used
/// by `PluginEditorBridge` to read and write processor order via the bridge API.
///
/// # Thread Safety
///
/// Implementations must be `Send + Sync`. The generated impl uses `AtomicU8`
/// stores for the pending-order slots and a `Mutex<Vec<u8>>` for the canonical
/// order state, making it safe to call from any thread.
pub trait ProcessorOrderAccess: Send + Sync {
    /// Returns the current processor order as slot indices (as strings).
    fn get_order(&self) -> Vec<String>;

    /// Sets the desired processor order.
    ///
    /// Returns `InvalidProcessorOrder` if the provided slice is not a valid
    /// permutation of the processor slot indices.
    fn set_order(&self, order: &[String]) -> Result<(), BridgeError>;
}
