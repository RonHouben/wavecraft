...
use quote::quote;
use syn::{parse_macro_input, Data, DeriveInput, Fields, Expr, ExprLit, Lit};
...
fn expand_derive(input: &DeriveInput) -> syn::Result<TokenStream> {
    ...
    for field in fields {
        ...
        let param_attr = field.attrs.iter()
...
fn parse_param_attr(field_name: &str, attr: &syn::Attribute) -> syn::Result<ParamSpecData> {
        ...
        match ident.to_string().as_str() {
            "range" => {
                ...
                if let Expr::Lit(ExprLit { lit: Lit::Str(lit_str), .. }) = value {
                    ...
                    if parts.len() == 2 {
                        range_min = Some(parts[0].trim().parse()
                            .map_err(|_| meta.error("Invalid range min"))?);
                        range_max = Some(parts[1].trim().parse()
    ...
    let min = range_min.ok_or_else(|| syn::Error::new_spanned(attr, "Missing 'range' attribute"))?;
    let max = range_max.ok_or_else(|| syn::Error::new_spanned(attr, "Missing 'range' attribute"))?;
