...
fn test_get_all_parameters_integration() {
    ...
    let result: wavecraft_protocol::GetAllParametersResult =
...
fn test_get_parameter_integration() {
    ...
    let result: wavecraft_protocol::GetParameterResult =
        serde_json::from_value(response.result.unwrap()).unwrap();
...
fn test_json_roundtrip() {
    ...
    // Parse response
    let response: wavecraft_protocol::IpcResponse = serde_json::from_str(&response_json).unwrap();
