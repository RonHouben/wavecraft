...
| Oscillator blocks DAW passthrough in generated projects                | **High priority bug.** In fresh `wavecraft create` projects, enabling `Oscillator` in the signal chain can mute incoming DAW signal in Ableton. Expected behavior: oscillator and incoming DAW audio are both audible simultaneously (layered/mixed), oscillator must not block passthrough. User stories: [oscillator-passthrough-mix](feature-specs/oscillator-passthrough-mix/user-stories.md).                                                                                                 |
