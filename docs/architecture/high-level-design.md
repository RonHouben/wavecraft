...
## Main components (concrete)
    ...
    •	**npm Package** (`@wavecraft/core`):
        - IPC: `IpcBridge`, `ParameterClient`, `MeterClient`, `SignalChainOrderClient`
        - Hooks: `useParameter`, `useAllParameters`, `useConnectionStatus`, `useMeterFrame`, `useHasProcessor`, `useAvailableProcessors`, `useSignalChainOrder`, `useOscilloscopeFrame`
    ...
    •	**TypeScript Codegen:** `wavecraft start` extracts metadata from the compiled Rust engine via FFI and generates `ui/src/generated/parameters.ts` and `ui/src/generated/processors.ts`. These files are regenerated automatically on Rust source changes during startup/hot-reload and are build artifacts (gitignored), not checked into source control.
...
```json
{
  ...
  "params": {
    ...
    "diagnostic": {
      "code": "noOutputDevice",
      "message": "No usable default output device"
    }
  }
}
```
...
{
  "jsonrpc": "2.0",
  "method": "signalChainOrderChanged",
  "params": {
    "slots": [
      { "id": "TestTone", "type": "processor" },
      { "id": "OscilloscopeTap", "type": "tap" },
