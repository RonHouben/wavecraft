...
The `wavecraft start` command extracts parameter IDs from the compiled plugin and generates a TypeScript module augmentation file at `ui/src/generated/parameters.ts`. The file is also refreshed by `wavecraft bundle` before the UI build/embedding step:

```typescript
// Auto-generated by `wavecraft start` — refreshed by `wavecraft bundle` — DO NOT EDIT
declare module '@wavecraft/core' {
  interface ParameterIdMap {
    __wavecraft_internal_augmented__: true;
    oscillator_enabled: true;
    oscillator_waveform: true;
    oscillator_frequency: true;
    oscillator_level: true;
    output_gain_level: true;
  }
}
...
This augments the `ParameterIdMap` interface in `@wavecraft/core`, causing the `ParameterId` conditional type to resolve to a literal union (for example: `'oscillator_enabled' | 'oscillator_waveform' | 'oscillator_frequency' | 'oscillator_level' | 'output_gain_level'`) instead of `string`. The result: IDE autocompletion and compile-time type checking for all `useParameter()` calls, `ParameterClient` methods, and component props — with zero developer effort.

The file is regenerated automatically on Rust source changes during development (via the hot-reload pipeline).

> **Runtime contract note:** Generated/runtime parameter IDs are canonical prefixed IDs (snake_case processor prefix + `id_suffix`) and should not use placeholder indexed IDs like `param_0..param_n`.

> **Stepped/enum parameters:** For stepped/enum parameters, runtime metadata must expose `ParameterType::Enum` with `variants` labels so UI enum controls can render meaningful option text.
