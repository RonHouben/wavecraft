...
## Regression Test Results — QA Fix Verification

**Date**: 2026-02-08
**Trigger**: Coder fixed 4 QA findings (Finding 1–4). This regression verifies fixes are correct and nothing else broke.

### RT-001: CI Passes After Fixes

**Description**: Full `cargo xtask ci-check` regression run.

**Status**: ✅ PASS

**Actual Result**: All checks passed in 10.5s:
- Engine (Rust): Formatting OK, Clippy OK, 162 tests passed (bridge 16, core 0+6, dev-server 9+9+6+3, dsp 15+3, macros 3, metering 8, nih_plug 2, protocol 15, xtask 42+16, doctests 12)
- UI (TypeScript): ESLint OK, Prettier OK, 28 Vitest tests passed across 6 test files
- No new warnings or errors introduced.

---

### RT-002: AudioHandle Lifetime Fix (Critical — Finding 1)

**Description**: Verify `AudioHandle` is stored in a named variable that lives until shutdown, not consumed by `.is_some()`.

**Status**: ✅ PASS

**Actual Result**: In `cli/src/commands/start.rs` lines 311–318:
```rust
...
```
- `_audio_handle` stores the `Option<AudioHandle>` in a named variable
- `.is_some()` is called separately on `_audio_handle`, NOT consuming it
- `_audio_handle` lives until end of `run_dev_servers()` scope — cpal stream stays alive
- Drop ordering is correct: `_audio_handle` (declared after `loader`) is dropped first (reverse local variable order), so `FfiProcessor` inside the closure drops while the Library in `loader` is still loaded
- Comment at lines 309–313 accurately documents this invariant

---

### RT-003: Drop Ordering Fix (Major — Finding 2)

**Description**: Verify `_library` is the LAST field in `PluginParamLoader` and docs are correct.

**Status**: ✅ PASS

**Actual Result**: In `engine/crates/wavecraft-bridge/src/plugin_loader.rs` lines 71–78:
```rust
...
```
- `_library` is the **last** field (line 77) — dropped last per Rust's "fields dropped in declaration order" rule
- Doc comment at lines 62–68 is now correct: states "Struct fields are dropped in declaration order (first declared = first dropped)" and "`_library` is the **last** field so it is dropped last"
- External invariant documented at lines 70–73: callers must ensure `FfiProcessor` is dropped before the loader
- Field-level doc comment on `_library` (lines 75–76) reinforces "must be the last field"

---

### RT-004: Channel Handling Fix (Medium — Finding 3)

**Description**: Verify audio callback reads actual channel count from stream config and handles mono/multichannel.

**Status**: ✅ PASS

**Actual Result**: In `engine/crates/wavecraft-dev-server/src/audio_server.rs` lines 88–120:
- Line 90: `let num_channels = self.stream_config.channels as usize;` — reads actual device channel count
- Line 100: `let num_samples = data.len() / num_channels.max(1);` — `.max(1)` prevents division by zero
- Lines 102–103: Guard against zero samples or zero channels: `if num_samples == 0 || num_channels == 0 { return; }`
- Lines 109–115: Mono handling: `if num_channels > 1` uses second channel for right; else duplicates left to right
- Comment at lines 88–90 documents the intent: "mono (1 ch), stereo (2 ch), and multi-channel (>2 ch) devices"

---

### RT-005: SAFETY Comments Fix (Minor — Finding 4)

**Description**: Verify all `unsafe` blocks in `plugin_loader.rs` have SAFETY comments.

**Status**: ✅ PASS

**Actual Result**: All `unsafe` blocks in `plugin_loader.rs` now have comprehensive `// SAFETY:` comments:
1. Lines 87–89: `Library::new()` — documents inherent unsafety of loading dynamic libraries
2. Lines 92–93: `library.get("wavecraft_get_params_json")` — documents symbol ABI contract
3. Lines 100–101: `library.get("wavecraft_free_string")` — documents symbol ABI contract
4. Lines 107–115: Combined FFI calls block — documents null check, `CStr::from_ptr` safety, and `free_string` deallocation matching
5. Lines 172–177: `library.get("wavecraft_dev_create_processor")` — documents graceful fallback via `.ok()?`
6. Lines 181–184: `symbol()` call — documents ABI match, no side effects, library validity

All SAFETY comments follow the coding standards pattern (explain why the operation is sound).

---

### RT-006: Template Validation

**Description**: Verify template is still clean after QA fixes.

**Status**: ✅ PASS

**Actual Result**:
- `cli/sdk-templates/new-project/react/engine/src/bin/` — directory does not exist ✅
- Template `Cargo.toml.template` — no `[features]`, `[[bin]]`, `cpal`, `env_logger`, `wavecraft-dsp`, `wavecraft-dev-server`, `anyhow`, or `tokio` entries ✅
...
|------|---------|----------|--------|
| RT-001 | CI Passes | — | ✅ PASS |
| RT-002 | AudioHandle lifetime | Critical | ✅ PASS |
| RT-003 | Drop ordering | Major | ✅ PASS |
| RT-004 | Channel handling | Medium | ✅ PASS |
| RT-005 | SAFETY comments | Minor | ✅ PASS |
| RT-006 | Template validation | — | ✅ PASS |

**Result**: All 4 QA findings verified fixed. No regressions introduced. 162 engine tests + 28 UI tests pass. Ready for QA sign-off.
