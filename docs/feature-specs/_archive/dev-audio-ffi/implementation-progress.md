...
- [x] QA-1 (Critical) — Store `AudioHandle` in `_audio_handle` binding so the cpal stream stays alive until shutdown
- [x] QA-2 (Major) — Reorder `PluginParamLoader` fields: `_library` is now the last field (dropped last), comment corrected
- [x] QA-3 (Medium) — Audio callback reads actual channel count from `stream_config.channels`; handles mono (duplicate to stereo) and >2 channels (use first 2)
- [x] QA-4 (Minor) — Added comprehensive `// SAFETY:` comments to all `unsafe` blocks in `plugin_loader.rs`
