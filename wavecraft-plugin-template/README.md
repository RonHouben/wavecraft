...
A VST3/CLAP audio plugin built with the [Wavecraft SDK](https://github.com/wavecraft/wavecraft).
...
│   │   ├── App.tsx
│   │   ├── components/
│   │   └── lib/wavecraft-ipc/  # Wavecraft IPC client
│   ├── package.json
│   └── vite.config.ts
└── README.md
```
...
| `ui/src/lib/wavecraft-ipc/` | IPC client for UI ↔ Engine communication |
| `engine/Cargo.toml` | Rust dependencies (Wavecraft SDK from GitHub) |
...
Your plugin uses these Wavecraft crates (via GitHub):

| Crate | Purpose |
|-------|---------|
| `wavecraft-core` | Main plugin framework, nih-plug integration, editor |
| `wavecraft-protocol` | Parameter definitions and IPC contracts |
| `wavecraft-dsp` | DSP traits and utilities |
| `wavecraft-bridge` | IPC handler (UI ↔ Audio communication) |
| `wavecraft-metering` | Real-time safe metering (SPSC ring buffer) |

### Key Traits

**`Processor` trait** (from `wavecraft-dsp`):
```rust
pub trait Processor {
    fn process(&mut self, buffer: &mut [&mut [f32]], transport: &Transport);
...
The template doesn't include hot reload by default. For development with hot reload, see the [Wavecraft standalone mode documentation](https://github.com/wavecraft/wavecraft#development-mode).

## Next Steps

- Read the [Wavecraft SDK documentation](https://github.com/wavecraft/wavecraft)
- Explore the [example plugins](https://github.com/wavecraft/wavecraft/tree/main/examples)
- Join the [Discord community](https://discord.gg/wavecraft) (if available)
...
**Built with [Wavecraft](https://github.com/wavecraft/wavecraft)** — Modern audio plugin framework for Rust + React
