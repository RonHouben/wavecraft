...
fn resolve_project_root(start_dir: &Path) -> Result<PathBuf> {
            ...
            fn run_nih_plug_bundle(engine_dir: &Path, package_name: &str) -> Result<()> {
                ...
                let bundle_result = nih_plug_xtask::main_with_args(package_name, args)
            ...
            fn resolve_vst3_bundle_path(engine_dir: &Path, package_name: &str) -> Result<PathBuf> {
                ...
                bail!(
                    ...
                    format!("{}.vst3", package_name),
                    bundled_dir_candidates(engine_dir)[0].display(),
                    bundled_dir_candidates(engine_dir)[1].display(),
                );
            }
            ...
            fn macos_vst3_install_dir() -> Result<PathBuf> {
                if !cfg!(target_os = "macos") {
                    bail!(
                        "Install failed: `wavecraft bundle --install` is currently supported on macOS only."
                    );
                }
            ...
            fn install_vst3_bundle_at_path(
                ...
                let file_name = src
                    .file_name()
                    .ok_or_else(|| anyhow::anyhow!("Install failed: invalid VST3 source path {}", src.display()))?;
    ...
    fn find_project_root_from_nested_directory() {
        ...
        fs::create_dir_all(root.join("engine")).expect("engine dir");
                ...
                fn install_reports_replace_failure_with_diagnostics() {
                    ...
                    fs::create_dir_all(&dest_bundle).expect("failed to create existing destination bundle fixture");
                ...
                fn create_fake_bundle(package_name: &str) -> PathBuf {
                    let bundle_dir = make_unique_temp_path("vst3-source-bundle").join(format!("{}.vst3", package_name));
                    fs::create_dir_all(bundle_dir.join("Contents")).expect("failed to create fake bundle directories");
                    fs::write(
